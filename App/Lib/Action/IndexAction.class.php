...
class IndexAction extends Action {
    ...
    public function imit() {
        ...
        echo "<h3><a href='" . __URL__ . "/counter' target='_blank'>Counter</a>   <a href='" . __URL__ . "/kv' target='_blank'>KVDB</a>  <a href='" . __URL__ . "/rank' target='_blank'>Rank</a>  <a href='" . __URL__ . "/mc' target='_blank'>Memcache</a>   <a href='" . __URL__ . "/tq' target='_blank'>TaskQueue</a>   <a href='" . __URL__ . "/storage' target='_blank'>Storage</a>    <a href='" . __URL__ . "/mail' target='_blank'>Mail</a>		<a href='" . __URL__ . "/attachmail' target='_blank'>AttachMail</a>   <a href='" . __URL__ . "/fetchurl' target='_blank'>fetchURL</a>  <a href='" . __URL__ . "/wrappers' target='_blank'> Wrappers</a> <a href='" . __URL__ . "/saeimage' target='_blank'>SaeImage</a>  <a href='" . __URL__ . "/saemysql' target='_blank'>SaeMysql</a></h3>";
    }
    ...
    public function upload() {
        if (!empty($_FILES)) {
            ...
            if (!$upload->upload()) {
                ...
                $this->assign('filename', $info[0]['savename']);//$info[0]['savename']为文件名
    ...
    public function sendMail(){
        ...
        $f = new SaeFetchurl();
        $img_data = $f->fetch( 'http://thinkfortest-public.stor.sinaapp.com/upload/thumb_1415701188.jpg');
        ...
        $new_data = $img->exec();
        $mail = new SaeMail();
        $mail->setAttach( array("my_picture.jpg" => $new_data ));
        $ret = $mail->quickSend('1434970057@qq.com', $subject, $content, '13510507252@163.com', 'wsxhq850687192');
        if ($ret === false) {
    ...
    public function attachmail(){
        ...
        if (!empty($_FILES)) {//
            ...
            if (!$upload->upload()) {
                ...
                $info = $upload->getUploadFileInfo();
                $this->assign('filename', $info[0]['savename']);//模版引用变量{$filename}
